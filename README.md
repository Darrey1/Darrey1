...
I build backend systems, AI agents, and device infrastructure for fintech, Web3, and connected hardware.

Currently building [Nexalware](https://www.nexalware.com/), an AIoT infrastructure platform connecting software and AI agents with physical devices.

## Technologies

`Go` `Python` `TypeScript` `gRPC` `FastAPI` `PostgreSQL` `MongoDB` `Redis` `Docker` `MQTT` `Solidity` `C++` `MCP`
